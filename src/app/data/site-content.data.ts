...
export const contentSections: readonly ContentSection[] = [
  ...
  },
  {
    ...
    intro: `Every season brings its own reason to celebrate, right in the backyard and around the neighborhood. Picnics, sports, and spring holidays keep friendships warm while the temperatures slowly rise. Soon, as the long days lead to playful long afternoons, it is perfect weather for the stargazing, while magical fireflies light up the yard. Then when the grocery seasonal aisle starts to trade tiki torches and grills for backpack and shoe sales, the apple orchards grow lush with sweet fruit. With the reds and oranges migrating from branch to the ground, loved ones circle the table under the dining room chandelier and think of all there is to be grateful for. As snow begins to tumble, the debates become on whether to go to Memorial Park by Vreeland to sled, or Third River Park behind the library. Outside stands a snowman happy snowman, hoping for a cup of that delicous hot cocoa.`,
    background: 'lavender',
    dotted: true,
    focalPhoto: {
